...
/** 本解析器需要的 DWARF 常量（DWARF4/5 共用）。 */
object DW {
    // TAG
    const val TAG_compile_unit = 0x11
    const val TAG_skeleton_unit = 0x4a // DWARF5 split dwarf 骨架
    ...
    const val TAG_abstract_origin = 0x31 // 这是 AT，占位防止误用
    const val TAG_entry_point = 0x1a
    ...
    const val AT_comp_dir_ = 0x1b
    const val AT_GNU_macros = 0x2111
    const val AT_rnglists = 0x2116
    ...
    const val AT_dwo_id = 0x2135 // GNU；DWARF5 = 0x7f 签名同义
    const val AT_GNU_dwo_id = 0x2135
    const val AT_signature = 0x69
    const val AT_type_signature = 0x69
    const val AT_type_offset = 0x7f
    ...
    // children flag
    const val CHILDREN_no = 0x00
    const val CHILDREN_yes = 0x01
    ...
    // rangelist entry kinds DWARF5 (.debug_rnglists)
    const val RLKE_end = 0x0
    const val RLKE_base_addressx = 0x1
    const val RLKE_startx_endx = 0x2
    const val RLKE_startx_length = 0x3
    const val RLKE_offset_pair = 0x4
    const val RLKE_base_address = 0x5
    const val RLKE_start_end = 0x6
    const val RLKE_start_length = 0x7
}
