...
/** DWARF tags / attributes / forms / line opcodes / rangelist opcodes.
 *  Only the subset the parser understands; unknown numeric values stay available as raw Ints. */
object DW_TAG {
    const val COMPILATION_UNIT = 0x11
    const val SUBPROGRAM = 0x2e
    const val INLINED_SUBROUTINE = 0x1d
    const val LEXICAL_BLOCK = 0x0b
    const val COMPILE_UNIT_SKELETON = 0x11
...
object DW_AT {
    ...
    const val DWO_ID = 0x2131
    const val STR_OFFSETS_BASE = 0x72
    const val STRP_SUP = 0x80
    const val ADDR_BASE = 0x73
    const val RNGLISTS = 0x63
    const val RNGLISTX = 0x61
    const val LOCATION = 0x02
    // GNU split dwarf (DWARF 4)
    const val GNU_DWO_NAME = 0x2130
    const val GNU_DWO_ID = 0x2134
    const val GNU_RANGES_BASE = 0x2148
}
