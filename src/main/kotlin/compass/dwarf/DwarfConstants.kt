...
object DW {
    ...
    const val AT_segment = 0x7900 // vendor marker used by our segmented fixtures
    /** Vendor extension: explicit ranking weight for overlapping functions. */
    const val AT_explicit_priority = 0x3e99
    ...
    const val LNS_set_address = 0x0d // DWARF5
    // extended
    ...
    // header content type codes (DWARF5 line / rnglist)
    const val CT_path = 0x1
    const val CT_directory_index = 0x2
    const val CT_timestamp = 0x3
    const val CT_size = 0x4
    const val CT_md5 = 0x5
    const val CT_directory = 0x1
    const val CT_rnglist_offset = 0x2

    // .debug_rnglists entry kinds (DWARF5)
    ...
    fun tagName(tag: Int): String = when (tag) {
        TAG_compile_unit, TAG_skeleton_unit, TAG_split_compile_unit, TAG_partial_unit -> "DW_TAG_compile_unit"
        TAG_subprogram -> "DW_TAG_subprogram"
        TAG_inlined_subroutine -> "DW_TAG_inlined_subroutine"
        else -> "DW_TAG_0x${tag.toString(16)}"
    }

    fun formName(form: Int): String =
        javaClass.declaredFields.firstOrNull { it.name.startsWith("FORM_") && it.getInt(this) == form }?.name
            ?: "DW_FORM_0x${form.toString(16)}"
}
