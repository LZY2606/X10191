...
object DW {
    ...
    const val TAG_entry_point = 0x03
    const val TAG_common_block = 0x16
    const val TAG_subrange_type = 0x21
    const val TAG_try_block = 0x26
    const val TAG_catch_block = 0x25
    const val TAG_GNU_call_site = 0x4109
    ...
    const val AT_GNU_linkage_name = 0x2007
    const val AT_rnglists_base = 0x73
    const val AT_addr_base = 0x70
    const val AT_str_offsets_base = 0x72
    const val AT_dwo_name = 0x76
    const val AT_dwo_id = 0x77
    ...
    const val AT_GNU_macros = 0x2119
    const val AT_compass_priority = 0x2001
    const val AT_namelist_item = 0x64
    ...
    const val RANGES_BASE_ADDRESS = -1L
}

/** 属性值：已解析为 Kotlin 值或标记为不可用（未知 form / 损坏）。 */
sealed class AttrVal {
    data class Num(val v: Long) : AttrVal()
    data class Str(val v: String) : AttrVal()
    data class Bytes(val v: ByteArray) : AttrVal()
    data class Flag(val v: Boolean) : AttrVal()
    data class Ref(val targetGlobalOffset: Long, val intraCuOffset: Long) : AttrVal()
    data class SectionOffset(val section: String, val offset: Long) : AttrVal()
    data class AddrIndex(val index: Long) : AttrVal()
    data class StrIndex(val index: Long) : AttrVal()
    data class RngListIndex(val index: Long) : AttrVal()
    data object Unsupported : AttrVal()
}

data class AttrSpec(val attr: Int, val form: Int)
data class AbbrevEntry(
    val code: Long,
    val tag: Int,
    val hasChildren: Boolean,
    val specs: List<Pair<AttrSpec, Long?>>,
)

data class DwarfSection(val name: String, val data: ByteArray, val fileOffset: Long)
